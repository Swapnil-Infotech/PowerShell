...
def assume_role(account_id, role_name="System_Admin"):
    ...
    The role_name should be consistent across all target accounts.
    """
    try:
        ...
        response = sts_client.assume_role(
            ...
            DurationSeconds=900,  # Session duration (in seconds)
        )
...
def get_windows_patch_baselines(session, region):
    """
    Fetches patch baseline details specifically for Windows OS using the provided 
    boto3 session and region.
    """
    try:
        ssm_client = session.client("ssm", region_name=region)
        paginator = ssm_client.get_paginator("describe_patch_baselines")
        
        # Filter for Windows operating system
